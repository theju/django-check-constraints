...
class CheckConstraintValidatorTests(TestCase):
    def testSQLGen1(self):
        ...
        c.check_name = "check_price"
        c.validate(opts)
        validators = opts.get_field_by_name('price')[0].validators
    ...
    def testSQLGen2(self):
        ...
        c.check_name = "check_name"
        c.validate(opts)
        validators = opts.get_field_by_name('name')[0].validators
        self.assertTrue(isinstance(validators[-1], LikeValidator))

    def testSQLGen3(self):
        c = Check(discount__between = [10, 20])
        c.check_name = "check_discount"
        c.validate(opts)
        validators = opts.get_field_by_name('discount')[0].validators
        self.assertTrue(isinstance(validators[-1], RangeValidator))

    def testSQLGen4(self):
        c = Check(gender__in = ("Male", "Female"))
        c.check_name = "check_gender"
        c.validate(opts)
        validators = opts.get_field_by_name('gender')[0].validators
        self.assertTrue(isinstance(validators[-1], ListValidator))

    def testSQLGen5(self):
        c = Check(mfg_date__gt = datetime(2010,1,1))
        c.check_name = "check_mfg_date"
        c.validate(opts)
        validators = opts.get_field_by_name('mfg_date')[0].validators
        self.assertTrue(isinstance(validators[-1], GTValidator))
    ...
    def testCascadedSQLGen(self):
        ...
        c.validate(opts)
        validators = opts.get_field_by_name('price')[0].validators
        self.assertTrue(isinstance(validators[-3], GTEValidator))
        self.assertTrue(isinstance(validators[-2], GTEValidator))
        self.assertTrue(isinstance(validators[-1], LTEValidator))
